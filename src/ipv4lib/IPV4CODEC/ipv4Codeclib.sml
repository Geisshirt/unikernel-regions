...
structure IPv4Codec :> IPV4_CODEC = struct
    ...
    fun verifyChecksumIPv4 header payload =
        let
            ...
            val checkSum : int =
                checksumHeader |> toByteList |> toHextets |> ipv4Checksum
        in
            if checkSum = header_checksum then (header, payload)
            else raise Fail "Bad checksum in incoming IPv4 packet."
        end
    ...
    fun encode (Header {
        ...
        let
            ...
            val header =
                String.substring (checksumHeader, 0, 10) ^
                intToRawbyteString checkSum 2 ^
                String.extract (checksumHeader, 12, NONE)
        in
            header ^ payload
        end
end
