structure TcpState : TCP_STATE = struct
    ...
    fun update_state statefun (CON {id, state, send_seqvar, receive_seqvar, receive_queue, retran_queue, dup_count, service_type}) = (
        CON {
            id = id,
    ...
    fun update_sseqvar ssvfun (CON {id, state, send_seqvar, receive_seqvar, receive_queue, retran_queue, dup_count, service_type}) = (
        CON {
            id = id,
            state = state,
    ...
    fun update_rseqvar rsvfun (CON {id, state, send_seqvar, receive_seqvar, receive_queue, retran_queue, dup_count, service_type})  = (
        CON {
            id = id,
            state = state,
            send_seqvar = send_seqvar,
    ...
    fun dup_inc (CON {id, state, send_seqvar, receive_seqvar, receive_queue, retran_queue, dup_count, service_type}) = (
        CON {
            id = id,
            state = state,
            send_seqvar = send_seqvar,
    ...
    fun dup_reset (CON {id, state, send_seqvar, receive_seqvar, receive_queue, retran_queue, dup_count = _, service_type}) = (
        CON {
            id = id,
            state = state,
            send_seqvar = send_seqvar,
    ...
    fun retran_enqueue {last_ack, payload} (CON {id, state, send_seqvar, receive_seqvar, receive_queue, retran_queue, dup_count, service_type}) =
        let
    ...
    fun rec_enqueue payload (CON {id, state, send_seqvar, receive_seqvar,receive_queue, retran_queue, dup_count, service_type}) =
        let
    ...
    fun rec_collect (CON {id, state, send_seqvar, receive_seqvar, receive_queue, retran_queue, dup_count, service_type}) = 
    ...
    fun retran_dequeue (CON {id, state, send_seqvar, receive_seqvar, receive_queue, retran_queue, dup_count, service_type}) = 
    ...
    fun retran_dropacked ack (CON {id, state, send_seqvar, receive_seqvar, receive_queue, retran_queue, dup_count, service_type}) =
        let fun drop q = 
                case dequeue q of 
    ...
    fun getSSV (CON {id = _, state = _, send_seqvar, receive_seqvar = _, receive_queue = _, retran_queue = _, dup_count = _ , service_type = _}) =
        let val SSV ssv = send_seqvar in ssv end

    fun getRSV (CON {id = _, state = _, send_seqvar = _, receive_seqvar, receive_queue = _, retran_queue = _, dup_count = _, service_type = _}) =
