...
struct
  ...
  fun add id (Fragment frag) (m : map) : map = 
    let val offset = (#offset frag)
        ...
        fun coolReplace [] = 
              (* Empty map. *)
              (id, if offset = 0 then length else 0, [Fragment frag]) :: m
              ...
              if k = id then
                let fun updatePrevFrags new_nxtoff [] nxtFrags = (new_nxtoff, nxtFrags)
                      | updatePrevFrags new_nxtoff (Fragment frag2 :: pfs) nxtFrags = 
                          ...
                          else (new_nxtoff, pfs @ (Fragment frag2 :: nxtFrags))
                    fun insertFrag prevFrags nxtFrags =
                          case nxtFrags of 
                            []  => updatePrevFrags nxtoff (Fragment frag::prevFrags) []
                          | (Fragment frag2 :: nfs) => 
                              (if offset >= (#offset frag2) + (#length frag2)
                              then updatePrevFrags 
                ...
                in 
                  (k, new_nxtoff, new_l) :: ms
                end 
              else 
                (k, nxtoff, l)::coolReplace ms
    in 
      ( Int.toString offset |> print;
        "\n" |> print;
        coolReplace m 
      )
    end
